...
Car.prototype.brake = function () {
  if (this.speed >= 5) this.speed = this.speed - 5;
  console.log(this.make + "速度是：" + this.speed);
};
...
const getDate = function () {
  return fetch(`https://zjhczl.github.io/yanzheng/index.html`)
    .then(
...
// pms.then((data) => alert(data)).catch((err) => alert("catch"));

//async await
const asc = async function () {
  try {
    if (Math.random() > 0.8) {
      throw new Error("err:>0.8");
    }
...
asc().then((data) => console.log(data));
